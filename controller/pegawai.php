...
class pegawai
{
	...
	public function updateFoto()
	{
		if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
			...
			if (isset($file_foto) && $file_foto['error'] === UPLOAD_ERR_OK) {
				$nama_file = $file_foto['name'];
				$lokasi_file = $file_foto['tmp_name'];

				// Mendapatkan ekstensi file
				$ekstensi = pathinfo($nama_file, PATHINFO_EXTENSION);
				// Memastikan ekstensi file adalah .jpg, .jpeg, atau .png
				if (in_array(strtolower($ekstensi), ['jpg', 'jpeg', 'png'])) {
					// Tentukan lokasi dan nama file baru sesuai NIP pegawai
					$file_baru = "lost/photo/" . $nip . ".jpg";

					// Pindahkan file foto ke lokasi baru
					if (move_uploaded_file($lokasi_file, $file_baru)) {
						// Jika berhasil, tampilkan pesan sukses
						echo  "<script> alert('Berhasil Update Photo');
					...
					echo "<script> 
							alert('Ekstensi file tidak valid. Hanya file dengan format JPG, JPEG, atau PNG yang diperbolehkan.');
							window.location = 'index.php?controller=pegawai&method=detail&nip=$nip'; 
							</script>";
				}
			} else {
				// Jika file foto tidak diupload, tampilkan pesan error
				echo "<script> 
						alert('Harap Pilih File Terlebih Dahullu');
						window.location = 'index.php?controller=pegawai&method=detail&nip=$nip'; 
						</script>";
			}
		}
...
function pelanggaran()
{
	...
	// DARI MODEL
	// MENGARAH KE METHOD DI CLASS MODEL PEGAWAI
	$data_insert = $this->pegawai->insertPelanggaran($nip, $jenis_pelanggaran, $tanggal_pelanggaran, $keterangan);

	// DARI VIEW
	if ($data_insert == TRUE) {
		echo "<script> 
				  window.location = 'index.php?controller=pegawai&method=pelanggaran&nip=$nip';
				  </script>";
	}
	// MENGARAHKAN KE FILE VIEW/INSERT_PELANGGARAN.PHP
	// JIKA HASIL PROSES INSERT GAGAL
	else {
		echo "<script> 
				  alert('Proses Insert Gagal!');
